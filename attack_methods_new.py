...
class Attack_None(nn.Module):
    ...
    def forward(self, inputs, targets, attack=None, batch_idx=-1):
        ...
        adv_loss = self.loss_func(outputs, targets)
        adv_loss = adv_loss.mean()
        #print(outputs.shape)
        return outputs, None, adv_loss
...
class Attack_PGD(nn.Module):
    ...
    def forward(self,
        ...
        #pdb.set_trace()
        targets_prob = F.softmax(logits_pred_nat.float(), dim=0)

        num_classes = targets_prob.size(0)
        #pdb.set_trace()
        #print(num_classes)
        ...
        y_sm = utils.label_smoothing(y_gt, y_gt.size(1), 0.7) 
        adv_loss = loss_ce(logits_pert, y_sm.detach())   

        #adv_loss = self.loss_func(logits_pert, y_tensor_adv)
        adv_loss = adv_loss.mean() 
        return logits_pert, targets_prob.detach(), adv_loss
