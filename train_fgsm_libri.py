...
from attack_methods_new import Attack_None, Attack_PGD
...
def train_fun(epoch, net, args, optimizer, trainloader, device):
    ...
    for batch_idx, (inputs, targets) in enumerate(iterator):
        ...
        # forward
        outputs, _, loss_fs = net(inputs.detach(), targets)
        optimizer.zero_grad()
        loss = loss_fs
        loss.backward()
        ...
        if batch_idx % args.log_step == 0:
            ...
            nat_outputs, _, _ = net(inputs, targets, attack=False)
...
def main(args):
    ...
    args.attack_method = attack_list[1]
...
def parse_args():
    ...
    parser.add_argument('--model_dir', default='/scratch/mp_323/gard-adversarial-audio/model_fgsm_moni/', type=str, help='model path')
    ...
    #parser.add_argument('--adv_mode', default='pgd', type=str, help='adv_mode (feature_scatter)')
